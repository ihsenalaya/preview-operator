...
\end{equation}
fitted with the \texttt{lme4}~\cite{bates_lme4_2015} convention via
\texttt{statsmodels} \texttt{MixedLM}. The
\texttt{configuration}:\texttt{LLM} interaction is significant at
\Cn{3} ($\beta = 0.067$, $p = 0.001$) and \Cn{4} ($\beta = 0.100$,
$p < 0.001$) on the unified 4000-row dataset (S1 + multi-app, both
LLMs). This is the cross-LLM consistency finding for \RQ{4}.
...
\paragraph{Aligned top-1, multi-app.} Pooled across
S2--S5, LLM-A grounded: $\mathbf{22.4\%}$ aligned top-1
(95\,\% Wilson CI [19.9\,\%, 25.1\,\%]); LLM-B grounded:
$\mathbf{10.4\%}$ (95\,\% Wilson CI [8.7\,\%, 12.4\,\%]). Per-subject
ranges: 17.6\,\% (umami) to 28.0\,\% (healthchecks).

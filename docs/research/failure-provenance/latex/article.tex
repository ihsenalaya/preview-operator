...
\paragraph{Agreement validation.} The pre-registered hallucination
annotation plan includes a 20\% shuffled agreement check between an
LLM-as-judge proxy and an independent reviewer. The current artifact
contains the proxy-judge pass, but the canonical human-reviewer column is
not yet complete. We therefore do not use Cohen's $\kappa$ to support any
headline claim in this version; rows that fail the planned $\kappa \geq
0.6$ threshold will be moved to a sensitivity table in the final
submission~\cite{mchugh_kappa_2012, inter_coder_se_2023}.
